...
func TryParseSeriesNumber(sr string) (labradar.SeriesNumber, bool) {
	if len(sr) != 6 {
		return labradar.SeriesNumber(0), false
	}
	if sr[0:2] != "SR" {
		return labradar.SeriesNumber(0), false
	}
	...
	return labradar.SeriesNumber(i), true
}
...
func WithSeriesNumber(n int) SeriesMutatorFn {
	return func(s *Series) {
		s.Number = labradar.SeriesNumber(n)
	}
}
