...
func Test_TrimLastChar(t *testing.T) {
	...
	tests := []struct {
		...
		{"Stripped the last NUL (unicode)", args{"Remove the unicode NUL" + constants.UnicodeNUL}, "Remove the unicode NUL"},
		{"Stripped the last NUL (hex)", args{"Remove the hex NUL" + constants.HexNUL}, "Remove the hex NUL"},
	}
