    ...
    public static class DialogHelper
    {
            ...
            bool selectMany = false)
        {
            ...
            var storageFiles = await topLevel.StorageProvider.OpenFilePickerAsync(
                ...
                {
                    AllowMultiple = selectMany, FileTypeFilter = fileTypes, Title = title ?? "Select any file(s)"
                });
        ...
        public static async Task<T?> ShowDialogWindow<T>(this IDialogParticipant? context,
            string windowTitle,
            object content,
        ...
        {
            context.ShowNotificationMessage(new Notification(title, message, notificationType,
        ...
        public static async Task<T?> ShowOverlayDialogAsync<T>(
            this IDialogParticipant? context,
            string title,
            object? content,
            params DialogCommand[] dialogCommands)
        {
            ...
            {
                Content = content, Header = title, DialogCommands = dialogCommands
            };
        ...
        /// <param name="result">The result to return</param>
        /// <exception cref="InvalidOperationException">If the <see cref="TopLevel" /> is not a <see cref="Window" /></exception>
        public static void ReturnResultFromOverlayDialog(this IDialogParticipant? context, object? result)
        {
            ArgumentNullException.ThrowIfNull(context);

            var overlayDialog = DialogManager.GetVisualForContext(context).FindAncestorOfType<OverlayDialog>(true);

            if (overlayDialog is null)
            {
                throw new InvalidOperationException("Unable to find OverlayDialog");
            }

            overlayDialog.Close(result);
        }
    }
}
