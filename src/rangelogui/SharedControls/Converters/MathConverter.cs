...
namespace SharedControls.Converters
{
    ...
    public class MathConverter : IValueConverter, IMultiValueConverter
    {
        ...
        object? Calculate(IList<object?> values, string operatorString)
        {
            ...
            double? GetDouble(object? value)
            {
                return value switch
                {
                    ...
                    string s => double.TryParse(s, CultureInfo.InvariantCulture, out var result)
                        ? result
                        : null, // Parse string
                    int i => i, // Convert int
