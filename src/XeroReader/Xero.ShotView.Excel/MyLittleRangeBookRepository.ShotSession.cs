                ...
                using (SqliteTransaction? transaction = conn.BeginTransaction(IsolationLevel.Serializable))
                {
                    try
                    {
                        const string COUNT_SQL =
                            "SELECT COUNT(*) AS kount FROM shotview_sessions WHERE name=@SheetName";
                        int rowsAffected = await conn
                            .ExecuteScalarAsync<int>(COUNT_SQL, new { session.SheetName }, transaction)
                            .ConfigureAwait(false);
                        int shotsAffected = 0;
                        if (rowsAffected < 1)
                        {
                            rowsAffected = await InsertSession(conn, transaction, session).ConfigureAwait(false);
                        }
                        else
                        {
                            session.Id = await GetSessionIdForName(conn, transaction, session.SheetName)
                                             .ConfigureAwait(false) ??
                                         throw new InvalidOperationException("Could not find a session named " +
                                                                             session.SheetName);
                            rowsAffected = await UpdateSession(conn, transaction, session).ConfigureAwait(false);
                        }
