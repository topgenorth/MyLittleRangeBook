...
namespace MyLittleRangeBook.Models
{
    public class MlrbIdTests
    {
        ...
        {
            var id = new MlrbId();
            var json = System.Text.Json.JsonSerializer.Serialize(id, Persistence.MlrbJsonSerializerContext.Default.MlrbId);
            var deserialized = System.Text.Json.JsonSerializer.Deserialize(json, Persistence.MlrbJsonSerializerContext.Default.MlrbId);
